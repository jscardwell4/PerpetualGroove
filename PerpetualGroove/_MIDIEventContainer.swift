...
struct _MIDIEventContainer: CollectionType {
...
//      ensureUnique()
  ...
  var metaEvents: LazyMapCollection<LazyFilterCollection<_MIDIEventContainer>, MetaEvent> {
    return lazy.filter({if case .Meta = $0 { return true } else { return false }})
               .map({$0.event as! MetaEvent})
  }

  var channelEvents: LazyMapCollection<LazyFilterCollection<_MIDIEventContainer>, ChannelEvent> {
    return lazy.filter({if case .Channel = $0 { return true } else { return false }})
               .map({$0.event as! ChannelEvent})
  }

  var nodeEvents: LazyMapCollection<LazyFilterCollection<_MIDIEventContainer>, MIDINodeEvent> {
    return lazy.filter({if case .Node = $0 { return true } else { return false }})
               .map({$0.event as! MIDINodeEvent})
  }

  var timeEvents: LazyMapCollection<LazyFilterCollection<_MIDIEventContainer>, MetaEvent> {
    return lazy.filter({
...
func ==(lhs: _MIDIEventContainer.Index, rhs: _MIDIEventContainer.Index) -> Bool {
