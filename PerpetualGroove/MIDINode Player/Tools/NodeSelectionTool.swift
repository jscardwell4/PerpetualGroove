...
class PresentingNodeSelectionTool: NodeSelectionTool, PresentingTool {
  ...
  {
    if dismissalAction == .cancel && MIDINodePlayer.undoManager.canUndo {
      if MIDINodePlayer.undoManager.groupingLevel > 0 { MIDINodePlayer.undoManager.endUndoGrouping() }
      MIDINodePlayer.undoManager.undo()
    }
    node = nil
  }
