...
final class Sequencer {
  ...
  static fileprivate var transportAssignment: TransportAssignment = Sequencer.primaryTransport {
    ...
    didSet {
      ...
      observe(transport: transportAssignment.transport)
  ...
  static fileprivate func observe(transport: Transport) {

    func bounceNotification(name: NotificationName) -> (Notification) -> Void {
      return {
        postNotification(name: name, object: Sequencer.self, userInfo: $0.userInfo)
      }
    }

    receptionist.observe(name: .didStart,        from: transport, callback: bounceNotification(name: .didStart))
    receptionist.observe(name: .didPause,        from: transport, callback: bounceNotification(name: .didPause))
    receptionist.observe(name: .didStop,         from: transport, callback: bounceNotification(name: .didStop))
    receptionist.observe(name: .didJog,          from: transport, callback: bounceNotification(name: .didJog))
    receptionist.observe(name: .didBeginJogging, from: transport, callback: bounceNotification(name: .didBeginJogging))
    receptionist.observe(name: .didEndJogging,   from: transport, callback: bounceNotification(name: .didEndJogging))
    receptionist.observe(name: .didReset,        from: transport, callback: bounceNotification(name: .didReset))
  }
