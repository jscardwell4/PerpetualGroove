...
struct MIDINodeHistory: Swift.Sequence {
 ...
 mutating func prune(after snapshot: Snapshot) {
  ...
  func snapshot(for ticks: MIDITimeStamp) -> Snapshot? {
    ...
    let trajectory = Trajectory(vector: breadcrumb.velocity, point: breadcrumb.position(for: ticks))
...
extension MIDINodeHistory {
  ...
  struct Snapshot {
    ...
    init(ticks: MIDITimeStamp, position: CGPoint, velocity: CGVector) {
      self.init(ticks: ticks, trajectory: Trajectory(vector: velocity, point: position))
    }

    init(ticks: MIDITimeStamp, trajectory: Trajectory) {
      self.ticks = ticks
