...
public protocol SoundFont2: CustomStringConvertible, JSONValueConvertible, JSONValueInitializable {
  ...
  subscript(idx: Int) -> PresetHeader { get }
  ...
  /// the same resource.
  func isEqualTo(_ soundSet: SoundFont2) -> Bool
}
...
//  var index: Int? { return Sequencer.soundFonts.firstIndex(where: {isEqualTo($0)}) }
  ...
  public subscript(position: Int) -> PresetHeader { return presetHeaders[position] }
  ...
  public var presetHeaders: [PresetHeader] {
    return (try? File.presetHeaders(from: data)) ?? []
  }
