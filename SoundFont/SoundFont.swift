...
public protocol SoundFont: CustomStringConvertible, JSONValueConvertible, JSONValueInitializable {
...
extension SoundFont {
  ...
  public static func ==(lhs: Self, rhs: Self) -> Bool { return lhs.url == rhs.url }
...
//  var index: Int? { return Sequencer.soundFonts.firstIndex(where: {isEqualTo($0)}) }
  ...
  public subscript(position: Int) -> SF2File.PresetHeader { return presetHeaders[position] }

  /// Returns the element in `presetHeaders` with matching values for `program` and `bank`.
  public subscript(program program: UInt8, bank bank: UInt8) -> SF2File.PresetHeader? {
    return presetHeaders.first(where: {$0.program == program && $0.bank == bank})
  }

  /// Returns `true` iff `other.url` is equal to `url`.
  public func isEqualTo(_ other: SoundFont) -> Bool {
  ...
  public var data: Data {
    guard let data = try? Data(contentsOf: url) else {
      fatalError("Failed to retrieve data from disk.")
    }
    return data
  }

  /// The collection of preset headers generated by parsing `data`.
  public var presetHeaders: [SF2File.PresetHeader] {
  ...
  /// an oscillator.
  public var image: UIImage { return #imageLiteral(resourceName: "oscillator") }

  /// Whether the sound font contains general midi percussion presets. The default is false.
  public var isPercussion: Bool { return false }

  /// The base name of the file located at `url`.
  public var fileName: String { return url.path.baseNameExt.baseName }

  /// The user-facing name of the sound font. The default is `fileName`.
  public var displayName: String { return fileName }

  public var description: String { return "\(displayName) - \(fileName)" }

}

/// A structure for creating a sound font with only a URL.
public struct AnySoundFont: SoundFont {
  ...
  public static let spyro = try! AnySoundFont(url: Bundle.main.url(forResource: "SPYRO's Pure Oscillators",
                                                                   withExtension: "sf2")!)
...
public struct EmaxSoundFont: SoundFont {
  ...
  public var url: URL { return Bundle.main.url(forResource: fileName, withExtension: "sf2")! }
  ...
  public var isPercussion: Bool { return volume == .drumsAndPercussion }
  ...
  public var fileName: String { return "Emax Volume \(volume.rawValue)" }
  ...
  public init(url: URL) throws {
    ...
    guard let volume = (~/"Emax Volume ([1-6])").firstMatch(in: url.path)?.captures[1]?.substring else {
    ...
    self.init(Volume(rawValue: Int(String(volume))!)!)
