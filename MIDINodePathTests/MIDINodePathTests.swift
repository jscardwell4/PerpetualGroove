...
final class MIDINodePathTests: XCTestCase {
  ...
  func testTrajectory() {
    ...
    expect(initialTrajectory.x) == initialPoint.x
    expect(initialTrajectory.y) == initialPoint.y
    expect(initialTrajectory.dx) == initialVelocity.dx
    expect(initialTrajectory.dy) == initialVelocity.dy
  ...
  func testSegmentForTime() {
    ...
    let segment1 = path.segment(for: time1)
    guard expect(segment1).toNot(beNil()) else { return }
    expect(segment1?.timeInterval.contains(time1)) == true

    let time2: BarBeatTime = 6∶3.210
    let segment2 = path.segment(for: time2)
    guard expect(segment2).toNot(beNil()) else { return }
    expect(segment2?.timeInterval.contains(time2)) == true

    let time3: BarBeatTime = 7∶3.261
    let segment3 = path.segment(for: time3)
    guard expect(segment3).toNot(beNil()) else { return }
    expect(segment3?.timeInterval.contains(time3)) == true
  }

  func testLocationForTime() {
    let path = MIDINodePath(trajectory: initialTrajectory, playerSize: playerSize, time: startTime)
    let location = path.location(for: 11∶1.116)
  ...
  func testSegmentGenerationPerformance() {
    ...
    measure { let _ = path.location(for: time) }
  }
