...
struct MIDIFile: ByteArrayConvertible, CustomStringConvertible {
  ...
  init(bytes: [UInt8]) {
    do {
     try self.init(data: Data(bytes))
      ...
      fatalError("\(#fileID) \(#function) Failed to initialize with `bytes`.")
    }
  }
  ...
  init(data: Data) throws {
    ...
    while tracksRemaining > 0 {
      ...
      guard String(bytes: data[currentIndex +--> 4]) == "MTrk" else {
  ...
  init(sequence: Sequence) {
    ...
    header = HeaderChunk(numberOfTracks: UInt16(tracks.count))
  ...
  struct TrackChunk: CustomStringConvertible {
    ...
    let type = UInt32(bytes: "MTrk".utf8)
...
extension Int {
  ...
  /// Initializing with MIDI file's variable length quantity.
  init(_ quantity: MIDIFile.VariableLengthQuantity) {
    self.init(bytes: quantity.bytes)
  }
