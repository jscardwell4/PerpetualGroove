...
public struct MetaEvent: _MIDIEvent, Hashable {
  ...
  public init(delta: UInt64, data: Foundation.Data.SubSequence) throws {
    ...
    guard data.count >= 3 else {
      ...
      throw Error.invalidLength("Not enough bytes in the data provided.")
    ...
    guard data[data.startIndex] == 0xFF else {
      ...
      throw Error.invalidHeader("The first byte of data must equal 0xFF")
    ...
    guard data.endIndex == currentIndex &+ dataLength else {
      ...
      // Throw an invalid length error.
      throw Error.invalidLength("Specified length does not match actual")
      ...
      switch type {
        ...
        case 0x2F:
          ...
          guard data.isEmpty else {
            ...
            throw Error.invalidLength("An end-of-track event has no data.")
