...
public struct BarBeatTime
{
  ...
  public init(tickValue: MIDITimeStamp,
              beatsPerBar: UInt = 4,
              beatsPerMinute: UInt = 120,
              subbeatDivisor: UInt = 480,
              isNegative: Bool = false)
  ...
  /// left on the bar, beat fraction, and subbeat fraction. The subbeat fraction,
  /// when converted to a mixed fraction, can be viewed as some number of beats and
  /// the remaining subbeats. To normalize these beats are added to the beat fraction's
  /// numerator and the remaining subbeats become the new subbeat fraction value. In
  /// the same way, converting the beat fraction to a mixed fraction yields a number
