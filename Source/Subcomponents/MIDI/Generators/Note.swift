...
public enum Note: RawRepresentable, Comparable, Hashable
{
  ...
  public var natural: Natural
  {
    get
    {
      switch self { case let .natural(n),
                         let .accidental(n, _): return n }
    }
  ...
  public var modifier: PitchModifier?
  {
    get
    {
      ...
      guard case let .accidental(_, modifier) = self else { return nil }
  ...
  public var flattened: Note
  {
    switch self
    {
      ...
      case let .accidental(natural, .sharp):
        // The note has been sharpened. Flatten by removing the modifier.
        
        return .natural(natural)
        
      case let .accidental(natural, .flat):
  ...
  public var sharpened: Note
  {
    switch self
    {
      ...
      case let .accidental(natural, .doubleFlat):
  ...
  public init?(rawValue: String)
  {
    ...
    guard let captures = (~/"^([A-G])([♭♯𝄫])?$").firstMatch(in: rawValue),
          ...
          let natural = Natural(rawValue: String(rawNatural))
    else { return nil }
  ...
  public static func < (lhs: Note, rhs: Note) -> Bool
  {
    ...
    {
      case let (.natural(natural1), .natural(natural2)),
           let
            (.accidental(natural1, _), .accidental(natural2, _)),
           let
            (.natural(natural1), .accidental(natural2, _)):
      ...
      case let (.accidental(natural1, _), .natural(natural2)):
