...
public struct MIDINote
{
  ...
  public static func normalizedNote(for note: Note) -> Note
  {
    switch note
    {
      case .natural(.c),
  ...
  public static let normalizedNotes: [Note] = [
    .natural(.c), .accidental(.c, .sharp), .natural(.d), .accidental(.d, .sharp),
    .natural(.e), .natural(.f),
    .accidental(.f, .sharp), .natural(.g), .accidental(.g, .sharp), .natural(.a),
    .accidental(.a, .sharp),
    .natural(.b)
  ]
  
  /// Returns the index of a note within the 12-note octave
  public static func index(for note: Note) -> Int
  { return normalizedNotes
    .firstIndex(of: normalizedNote(for: note))!
  }
  
  public init(_ note: Note, _ octave: Octave)
  {
    self.note = note
    self.octave = octave
  }
  ...
  {
    return UInt8((octave.rawValue + 1) * 12 + MIDINote.index(for: note))
  }
  
  public var percussionMapping: PercussionMapping?
  {
    return PercussionMapping(rawValue: midi)
  }
}
...
extension MIDINote: RawRepresentable, LosslessJSONValueConvertible
{
  ...
  public init?(rawValue: String)
  {
    guard let captures = (~/"^([A-G][♭♯𝄫]?) ?((?:-1)|[0-9])$").firstMatch(in: rawValue),
          ...
          let octave = Octave(rawValue: rawOctave)
    else { return nil }
  ...
  public var rawValue: String { return "\(note.rawValue)\(octave.rawValue)" }
}
...
{
  public var hashValue: Int { return midi.hashValue }
