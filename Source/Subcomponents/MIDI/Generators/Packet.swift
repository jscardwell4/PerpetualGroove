...
// MARK: - MIDIPacketList + Swift.Sequence
...
public struct Packet: CustomStringConvertible
{
  ...
  /// a `MIDIPacketList`.
  public func packetList(ticks: UInt64) -> MIDIPacketList
  {
    // Create the list.
    ...
    MIDIPacketListAdd(&packetList, size, packet, ticks, data.count, data)
  ...
  public init?(packetList: UnsafePointer<MIDIPacketList>)
  {
    ...
    identifier =
      UInt64(bytes: [packet.data.3, packet.data.4, packet.data.5, packet.data.6])
  }
