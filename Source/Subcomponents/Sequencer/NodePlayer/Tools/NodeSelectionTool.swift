...
public class NodeSelectionTool: Tool {
  ...
  private func addLighting(to node: Node) {
    ...
    guard node.childNode(withName: NodeSelectionTool.nodeLightingName) == nil else {
      return }
  ...
  private func removeLighting(from node: Node) {
    ...
    guard let light = node.childNode(withName: NodeSelectionTool.nodeLightingName) else {
      return }
  ...
  /// Handles registration/reception of notifications from `NodePlayer`.
  private let receptionist = NotificationReceptionist(callbackQueue: .main)

  /// Initialize with a player node. Registers for node addition/removal notifications from `NodePlayer`.
  public init(playerNode: PlayerNode) {

    self.playerNode = playerNode

    receptionist.observe(name: .didAddNode, from: player,
                         callback: weakCapture(of: self,
                                               block:NodeSelectionTool.didAddNode))
    receptionist.observe(name: .didRemoveNode, from: player,
                         callback: weakCapture(of: self,
                                               block:NodeSelectionTool.didRemoveNode))
  ...
  public func didAddNode(_ notification: Notification) {}

  /// Handler for notifications from `NodePlayer` that a nodes has been removed.
  /// The default implementation does nothing.
  public func didRemoveNode(_ notification: Notification) {}
  ...
  public func previousNode() {
    ...
    guard let index = playerNode.midiNodes.firstIndex(where: {$0 === node}) else {
      return }
...
public class PresentingNodeSelectionTool: NodeSelectionTool, PresentingTool {
  ...
  public override func didSelectNode() {
  ...
  /// to the player.
  public override func didAddNode(_ notification: Notification) {
  ...
  /// than two nodes.
  public override func didRemoveNode(_ notification: Notification) {
