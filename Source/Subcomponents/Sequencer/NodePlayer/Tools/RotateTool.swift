...
public final class RotateTool: PresentingNodeSelectionTool {
  ...
  private func didRotate(_ rotation: CGFloat) {
    ...
    guard let node = node,
          node.initialTrajectory.angle != rotation
    else { return }
...
public final class RotateViewController: UIViewController, SecondaryContent {
  ...
  fileprivate static func viewController(
  ...
  /// and `confirm` actions.
  public var supportedActions: SecondaryControllerContainer.SupportedActions =
    [.cancel, .confirm]
  ...
  @IBOutlet private var ball: TemplateImageView!

  /// The arrow indicating the angle of rotation.
  @IBOutlet private var arrow: TemplateImageView!

  /// The gesture used to change the angle of rotation.
  @IBOutlet private var rotationGesture: UIRotationGestureRecognizer!

  /// The callback invoked when `rotationGesture` indicates a change to the
  /// angle of rotation.
  ///
  ...
  private func handleRotation(_ sender: UIRotationGestureRecognizer) {
    switch sender.state {
      ...
      case .changed:
        // The gesture's rotation has been updated. Synchronize the arrow's
        // transform with the gesture's rotation and invoke the callback using
        // the updated value.
      ...
      case .cancelled,
           ...
           .possible,
           .ended:
        // Nothing to do since there is either nothing to report or the
        // callback has already been invoked when the gesture reported
