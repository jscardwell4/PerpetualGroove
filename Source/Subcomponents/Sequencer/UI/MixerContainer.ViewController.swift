//
//  MixerContainer.ViewController.swift
//  PerpetualGroove
//
...
public extension MixerContainer
{
  /// Controller for displaying the mixer interface.
  final class ViewController: UICollectionViewController, SecondaryContentProvider
  {
    ...
    fileprivate weak var sequence: Sequencer.Sequence?
    {
      didSet
      {
        ...
        if let sequence = sequence
        {
          ...
          let callback = weakCapture(of: self, block: MixerContainer.ViewController.updateTracks)
    ...
    override public func numberOfSections(in _: UICollectionView) -> Int { return Section.sections.count }
  ...
  {
    fileprivate typealias Section = MixerContainer.ViewController.Section
