...
//          Sequence.current?.currentTrack?.instrument.playNote($0)
  ...
  /// Overridden to deselect the tool when it is the current tool of `NodePlayer` and `mode == .new`
  /// in addition to actions performed in `super.didHide(content:dismissalAction:)`.
  public override func didHide(content: SecondaryContent, dismissalAction: DismissalAction) {
