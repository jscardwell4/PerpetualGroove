...
public final class RemoveTool: Tool {
  ...
  public init(playerNode: PlayerNode, delete: Bool = false) {
    ...
    // Register for notifications from `NodePlayer`.
    receptionist.observe(name: .willChangeDispatch, from: Player.self,
                         callback: weakCapture(of: self, block: RemoveTool.willChangeDispatch))
    receptionist.observe(name: .didChangeDispatch, from: Player.self,
  ...
  /// Handles registration and reception of notifications posted by `NodePlayer`.
  ...
  private func dispatchNodes(at point: CGPoint) -> [Node] {
    ...
    let identifiers = Set(player.nodes(at: point).compactMap({($0 as? Node)?.identifier}))
