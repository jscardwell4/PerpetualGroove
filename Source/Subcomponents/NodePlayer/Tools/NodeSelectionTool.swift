...
public class NodeSelectionTool: Tool {
  ...
  public unowned let player: PlayerNode
  ...
  public init(playerNode: PlayerNode) {
    ...
    receptionist.observe(name: .didAddNode, from: Player.self,
                         callback: weakCapture(of: self, block:NodeSelectionTool.didAddNode))
    receptionist.observe(name: .didRemoveNode, from: Player.self,
  ...
  private func refreshNode() {
    ...
    guard player.contains(point),
          let node = player.nodes(at: point).first(where: {$0 is Node}) as? Node
      else
    {
      return
    }
