...
/// Protocol for objects that can be set to handle touches received by a `PlayerNode` instance.
...
public final class PlayerNode: SKShapeNode {
  ...
  /// Initialize with the shape defined by `bezierPath`. Sets `NodePlayer.playerNode` to `self`.
...
//  var defaultNodes: [Node] { return midiNodes(for: .default) }

  /// The collection of midi nodes for loop mode.
//  var loopNodes: [Node] { return midiNodes(for: .loop) }
...
//  private func midiNodes(for mode: Sequencer.Mode) -> [Node] {
//    return self["<\(mode.rawValue)>*"].compactMap({$0 as? Node})
//  }
