...
/// Class for managing a collection of `MIDINode` instances for an owning
/// `MIDINodeDispatch` object.
public final class MIDINodeManager {

  /// The type for an event adding or removing a `MIDINode`.
  private typealias Event = MIDINodeEvent

  /// The object responsible for the manager.
  public unowned let owner: MIDINodeDispatch

  /// Default initializer.
  public init(owner: MIDINodeDispatch) { self.owner = owner }

  /// The set of nodes currently being managed.
  public private(set) var nodes: OrderedSet<MIDINodeRef> = []
...
//    logi("adding node \(node.name!) (\(node.identifier))")
  ...
  public func handle(event: MIDINodeEvent) {
