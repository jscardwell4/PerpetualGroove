...
/// A view controller for presenting a list of existing documents and a control for
/// creating a new document. Selecting an existing document in the list will load its
/// sequence and update the bookmark data stored in settings.
///
...
public final class DocumentsViewController: UICollectionViewController
{
  ...
  public func deleteItem(_ sender: LabelButton)
  {
    ...
    // Ensure the item manager can remove `item`, returning the index from which
  ...
  ///
  /// When the controller's view is loaded, setting this property causes the
  /// collection view to select the item at this index path or clear the selection
  ...
  private var selectedItem: IndexPath?
  {
    didSet
    {
      ...
      collectionView?.selectItem(at: selectedItem,
                                 animated: true,
  ...
  private func itemCountDidChange(from _: Int, to _: Int)
  {
  ...
  {
    CGFloat(itemManager.count &+ 1) * DocumentsViewController.itemHeight
  }

  /// Uses the value in `Manager.currentDocument` to locate the corresponding element
  /// in `items`, setting `selectedItem` to its equivalent index path. If the
  /// collection does not contain such an item, it is created, a new cell is added to
  /// the collection view, and then `selectedItem` updated.
  private func refreshSelection()
  {
    ...
    {
      case let index?:
        // Update `selectedItem` with an index path consisting of `index`
        // and the document item section.
  ...
  /// Overridden to add `collectionView` constraints and update
  /// `collectionViewLayout.controller`.
  override public func viewDidLoad()
  {
    ...
    widthConstraint =
      (collectionView.width == itemWidth --> (id + "Width")).activeConstraint
    heightConstraint =
      (collectionView.height == collectionHeight --> (id + "Height")).activeConstraint
  ...
  override public func numberOfSections(in _: UICollectionView) -> Int { 2 }
  ...
  -> UICollectionViewCell
  {
    ...
    switch indexPath.section
    {
      case 0:
        ...
        cell = collectionView
          .dequeueReusableCell(withReuseIdentifier: "CreateDocumentCell",
                               for: indexPath)
      ...
      case 1:
        ...
        cell = collectionView
          .dequeueReusableCell(withReuseIdentifier: "DocumentCellNib",
                               for: indexPath)
...
extension DocumentsViewController
{
  ...
  final class ItemManager: Collection
  {
    ...
    func makeIterator() -> Iterator { items.makeIterator() }

    func dropFirst(_ n: Int) -> SubSequence { items.dropFirst(n) }
    func dropLast(_ n: Int) -> SubSequence { items.dropLast(n) }
    func prefix(_ maxLength: Int) -> SubSequence { items.prefix(maxLength) }
    func suffix(_ maxLength: Int) -> SubSequence { items.suffix(maxLength) }
    func split(maxSplits: Int,
               omittingEmptySubsequences: Bool,
               whereSeparator isSeparator: (DocumentItem) throws -> Bool)
    rethrows -> [SubSequence]
    {
      try items.split(maxSplits: maxSplits,
    ...
    var indices: Indices { items.indices }
    func index(after i: Index) -> Index { items.index(after: i) }

    subscript(position: Index) -> DocumentItem { items[position] }
    subscript(bounds: Range<Index>) -> SubSequence { items[bounds] }
    ...
    @discardableResult
    func remove(at index: Index) -> DocumentItem { items.remove(at: index) }
    ...
    private func didUpdateItems(notification: Notification)
    {
      ...
      {
        case 0-->:
          // More items have been added than removed. Configure variables
          // for cell insertion.
        ...
        case <--0:
          // More items have been remove than added. Configure variables for cell
          // deletion.
      ...
      let updates = {
        ...
        // Perform the stored action using index paths derived from the stored upper
        // and lower bounds.
        performAction(collectionView)(bounds.map { IndexPath(item: $0, section: 1) })
      }

      // Perform the batched updates, refreshing the controller's selection upon
    ...
    private func documentDidChangeName(notification: Notification)
    {
      ...
      guard let index = items.index(of: newItem)
      else
      {
      ...
      assert(items[index].name == newName,
             "Item was not actually replaced in ordered set.")

      // Create an index path for the document's cell.
      let indexPath = IndexPath(item: index, section: 1)

      // Retrieve the existing cell for the document.
      guard let cell = controller?.collectionView?
              .cellForItem(at: indexPath) as? DocumentCell
      else
      {
        return
      }
...
/// Custom layout for `Documents.ViewController`.
public final class DocumentsViewLayout: UICollectionViewLayout
{
  ...
  fileprivate weak var controller: Documents.DocumentsViewController?
  ...
  {
    // Calculate the item count as the number of items specified by the controller
    // plus one for the document creation cell.
    let itemCount = (controller?.itemCount ?? 0) &+ 1

    // Create a collection to store the tuples that will be used to create
    // `attributesCache`.
    var tuples = ContiguousArray<(key: IndexPath,
                                  value: Attributes)>(minimumCapacity: itemCount)

    // Generate an array of all valid index paths for the collection.
    let indexPaths = [IndexPath(item: 0, section: 0)] + (0 ..< itemCount)
      .map { IndexPath(item: $0, section: 1) }

    // Iterate through the index paths.
    for indexPath in indexPaths
    {
      // Generate attributes for `indexPath` and append the `(indexPath, attributes)`
      // tuple to `tuples`.
  ...
  {
    // Get the frame from the last attributes as it will have the max y value furthest
    // from zero.
  ...
  -> UICollectionViewLayoutAttributes?
  {
    ...
    let size = CGSize(width: controller?.itemWidth
                        ?? Documents.DocumentsViewController.minimumItemWidth,
                      height: Documents.DocumentsViewController.itemHeight)
    ...
    {
      case 0:
        // The index path points to the document creation section. This section has
        // one cell that is always located at the collection's origin.

        origin = .zero

      case 1:
        // The index path points to the section of document items. The y offset for
        // the frame is the item specified by the index path multiplied by the cell
        // height plus the height of the document creation cell.
...
/// A cell for displaying the name of an existing document with a control for
...
public final class DocumentCell: UICollectionViewCell
{
  ...
  public var isShowingDelete: Bool { leadingConstraint?.constant ?? 0 < 0 }

  /// The item specifying the document to associate with the cell.
  public var item: DocumentItem? { didSet { label.text = item?.name } }

  /// Returns the duration for an animation traveling `distance` or `0.25`
  /// when `distance == nil`.
  private func animationDurationForDistance(_ distance: CGFloat?) -> TimeInterval
  {
    TimeInterval(CGFloat(0.25)
                  * (distance ?? deleteButton.bounds.width) / deleteButton.bounds.width)
  }

  /// Animates the cell to reveal the cell's delete button. Sets `isShowingDelete`
  /// to `true`.
  ...
  /// Animates the cell to hide the cell's delete button. Sets `isShowingDelete`
  /// to `false`.
  ...
  /// Handles the pan gesture attached to the cell for showing and hiding the
  /// delete button.
  @IBAction
  private func handlePan(_ gesture: PanGesture)
  {
    ...
    switch (gesture.state, x)
    {
      ...
      case (.began, 0-->),
           (.changed, 0-->):
        // Moving content right. Update the leading constraint using the x value
        // minus the width of the delete button.

        leadingConstraint.constant = -deleteButton.bounds.width + x

      case (.ended, ...(-deleteButton.bounds.width)):
        // Ended with the delete button showing. Fully reveal the delete button
        // animating with a distance derived from the x value.

        revealDelete(abs(x))

      default:
        // Canceled or ended with delete button hidden. Fully hide the delete
        // button animating with a distance derived from the x value.
