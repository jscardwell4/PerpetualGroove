...
/// nodes.
public final class InstrumentTrack: Track, MIDINodeDispatch, Hashable {

  /// The currently selected track in the current sequence or `nil`.
  public static var current: InstrumentTrack? { return Sequence.current?.currentTrack }
  ...
  public override func validate(events container: inout MIDIEventContainer) {
  ...
  public override func add<Source>(events: Source)
  ...
  public override var headEvents: [MIDIEvent] {
  ...
  public var nextNodeName: String { return "\(displayName) \(nodeManager.nodes.count + 1)" }
                    ...
                    context: UnsafeMutableRawPointer?)
  {
    ...
    eventQueue.async {
      [weak self, time = Time.current?.barBeatTime ?? .zero] in
  ...
  public override func registrationTimes<Source>(forAdding events: Source) -> [BarBeatTime]
    where Source:Swift.Sequence, Source.Iterator.Element == MIDIEvent
  {
  ...
  public init(sequence: Sequence, instrument: Instrument) throws {
    ...
    color = TrackColor.nextColor(currentColors: Set(sequence.instrumentTracks.map(\.color)))
