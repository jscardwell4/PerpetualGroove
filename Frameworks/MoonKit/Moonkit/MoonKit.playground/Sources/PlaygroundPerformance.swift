...
public func measure(name:String? = nil, iterations:Int = 10, forBlock block:MeasuredBlock) -> Double {
    ...
    for _ in 0..<iterations{
        ...
        if let name = name {
          XCPlaygroundPage.currentPage.captureValue(took, withIdentifier: name)
        }
    ...
    if let name = name {
      ...
      XCPlaygroundPage.currentPage.captureValue(mean.milliSeconds, withIdentifier: "\(name) Average")
      XCPlaygroundPage.currentPage.captureValue(variance.milliSeconds, withIdentifier: "\(name) STD Dev.")
    }
