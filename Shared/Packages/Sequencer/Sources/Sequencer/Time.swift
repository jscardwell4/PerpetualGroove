...
public final class Time: ObservableObject, Named, CustomStringConvertible, Hashable
{
  ...
  /// Callback invoked from MIDI Services thread when the time's MIDI clock source
  /// sends out MIDI packets.
  ...
  @Published public var barBeatTime = BarBeatTime.zero
  {
    didSet
    {
  ...
  /// The type registerable as a predicated callback. The tuple consists of a `predicate`
  /// to evaluate and a `callback` to execute when `predicate` returns `true`.
  ...
  /// The index of predicated callbacks registered with the time. Predicated callbacks
  /// are stored under a unique identifier to support lookup and removal operations.
  ...
  private func invokeCallbacks(for time: BarBeatTime)
  {
    ...
    guard !(suppressCallbacks || callbacks.isEmpty && predicatedCallbacks.isEmpty)
    else
    {
      return
    }
  ...
  /// Removes the callback registered for `time` with `identifier`.
  /// If `identifier` is `nil`, removes all callbacks registered for `time`.
  ...
  public init(clockSource: MIDIEndpointRef)
  {
    ...
    guard let name = unmanagedName?.takeUnretainedValue() as String?
    else
    {
    ...
    do
    {
      ...
      try require(MIDIInputPortCreateWithBlock(client, "Input" as CFString,
                                               &inPort, read),
                  "Failed to create input port.")
      
      // Connect the clock source to the time's input port.
      try require(MIDIPortConnectSource(inPort, clockSource, nil),
  ...
  deinit
  {
    do
    {
      ...
      try require(MIDIPortDispose(inPort), "Failed to dispose of the input port.")
      
      // Dispose of the MIDI client.
      try require(MIDIClientDispose(client), "Failed to dispose of the MIDI client.")
    }
    catch
    {
      // Just log the error.
  ...
  public var description: String { barBeatTime.description }
  
  /// Returns `true` iff `ObjectIdentifier` instance intialized with `lhs`
  /// and `rhs` are equal.
