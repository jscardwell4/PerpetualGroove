...
public final class InstrumentTrack: Track, NodeDispatch, ObservableObject, Identifiable
{
  ...
  {
    get { (Sequencer.shared.player.currentDispatch as? InstrumentTrack) === self }
    set
    {
      switch (isCurrentDispatch, newValue)
      {
        case (false, false):
          break
        case (false, true):
          Sequencer.shared.player.currentDispatch = self
        case (true, false):
          Sequencer.shared.player.currentDispatch = nil
        case (true, true):
          break
      }
    }
  ...
  {
    Sequencer.shared.mode == .linear && Sequencer.shared.player.currentDispatch === self
  }
  ...
  public init(index: Int,
              ...
              events: [Event]) throws
  {
    instrument = try Instrument(preset: preset, audioEngine: Sequencer.shared.audioEngine)
    self.color = color
    super.init(index: index)
    self.name = name
    add(events: events)
  ...
  public init(index: Int, trackChunk: TrackChunk) throws
  {
    ...
    else
    {
      instrument = try Instrument(preset: Sequencer.shared.auditionInstrument.preset,
                                  audioEngine: Sequencer.shared.audioEngine)
    }
  ...
  private func setup() throws
  {
    ...
    subscriptions.store
    {
      Sequencer.shared.transport.didResetPublisher
      ...
      Sequencer.shared.player.$currentDispatch.sink
      {
        self.isCurrentDispatchSubject.send(($0 as? InstrumentTrack) === self)
      }
      Sequencer.shared.player.$currentDispatch.combineLatest(Sequencer.shared.$mode).sink
      {
