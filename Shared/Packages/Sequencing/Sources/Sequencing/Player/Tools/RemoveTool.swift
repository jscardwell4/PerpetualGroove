...
public final class RemoveTool: Tool
{
  ...
  {
    guard let dispatch = Sequencer.shared.player.currentDispatch else { return }
  ...
  private func willChangeDispatch(_: Foundation.Notification)
  {
    ...
    Sequencer.shared.player.currentDispatch?.nodes.forEach(addBackgroundLight)
  }
  ...
  private func didChangeDispatch(_: Foundation.Notification)
  {
    touch = nil
    guard active else { return }
    Sequencer.shared.player.currentDispatch?.nodes.forEach(addForegroundLight)
  }
  ...
  public init(playerNode: PlayerNode, delete: Bool = false)
  {
    ...
    dispatchSubscription = Sequencer.shared.player.$currentDispatch.sink(receiveValue: {
      [self] (newValue: NodeDispatch?) in
      touch = nil
      guard active else { return }
      Sequencer.shared.player.currentDispatch?.nodes.forEach(addBackgroundLight(to:))
    ...
    addNodeSubscription = NotificationCenter.default
      .publisher(for: .playerDidAddNode, object: Sequencer.shared.player)
      .sink(receiveValue: {
        ...
        if Sequencer.shared.player.currentDispatch === dispatch
        {
  ...
  private func removeMarkedNodes()
  {
    do
    {
      ...
      guard let manager = Sequencer.shared.player.currentDispatch?.nodeManager else { return }
  ...
  private func dispatchNodes(at point: CGPoint) -> [MIDINode]
  {
    ...
    guard let dispatchNodes = Sequencer.shared.player.currentDispatch?.nodes else { return [] }
