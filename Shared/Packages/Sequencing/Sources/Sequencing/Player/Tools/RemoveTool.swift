...
@available(OSX 10.15, *)
final class RemoveTool: TouchReceiver
{
  @Environment(\.currentDispatch) var currentDispatch: NodeDispatch?
  ...
  {
    for node in player.playerNode.midiNodes
    {
  ...
  {
    guard let dispatch = currentDispatch else { return }

    let dispatchNodes = dispatch.nodeManager.nodes.compactMap { $0.reference }

    let (foregroundNodes, backgroundNodes) = player.playerNode.midiNodes.compactMap { $0 }.bisect
    {
  ...
  private func willChangeDispatch(_: Foundation.Notification)
  {
    ...
    currentDispatch?.nodes.forEach(addBackgroundLight)
  }
...
//    addNodeSubscription = player.publisher(for: .lastAdded)
//      .sink(receiveValue: {
//        [self] node in
//
//        guard active,
...
//      })
  ...
  private func removeMarkedNodes()
  {
    do
    {
      ...
      guard let manager = currentDispatch?.nodeManager else { return }
  ...
  private func dispatchNodes(at point: CGPoint) -> [MIDINode]
  {
    ...
    guard let dispatchNodes = currentDispatch?.nodes else { return [] }

    // Get the identifiers for all the midi nodes located at `point`.
    let identifiers = Set(player.playerNode.nodes(at: point)
  ...
  @objc func touchesBegan(_ touches: Set<UITouch>)
  {
    ...
    guard let point = touch?.location(in: player.playerNode),
          player.playerNode.contains(point)
    else { return }
  ...
  @objc func touchesMoved(_ touches: Set<UITouch>)
  {
    ...
    let point = touch!.location(in: player.playerNode)

    // Check that the player contains the point; otherwise, stop tracking `touch`.
    guard player.playerNode.contains(point)
    else
    {
      touch = nil
      return
