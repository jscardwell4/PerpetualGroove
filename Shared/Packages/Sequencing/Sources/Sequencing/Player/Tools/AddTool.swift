...
@available(OSX 10.15, *)
final class AddTool: TouchReceiver
{
  ...
  private func updateData()
  {
    ...
    guard let timestampʹ = touch?.timestamp,
          let locationʹ = touch?.location(in: player.playerNode),
  ...
  @objc func touchesEnded(_ touches: Set<UITouch>)
  {
    ...
    player.placeNew(trajectory: trajectory, target: dispatch, generator: generator)
  ...
  @objc func touchesMoved(_ touches: Set<UITouch>)
  {
    ...
    guard let position = touch?.location(in: player.playerNode),
          player.playerNode.contains(position)
    else
    {
      touch = nil
      return
