...
public final class MIDINode: SKSpriteNode
{
  ...
  private func didEndJogging(notification: Notification)
  {
    ...
    guard let nextSegment = path.segmentIndex(for: Sequencer.shared.time.barBeatTime)
    else
    {
              ...
              identifier: UUID = UUID()) throws
  {
    ...
    jogging = Sequencer.shared.transport.isJogging
    ...
    guard let playerSize = Sequencer.shared.player.playerNode?.size
    else
    {
